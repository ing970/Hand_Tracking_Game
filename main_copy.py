...
class Spark():
    ...
    def point_towards(self, angle, rate):
        ...
        rotate_sign = abs(rotate_direction) / rotate_direction if rotate_direction else 1
...
def simultaneous_notes():
    global beat_times, t1, t2, t3, t4 # 전역 변수 선언
    total_beat = len(beat_times) # beat_times 리스트의 길이를 total_beat 변수에 저장
    sample = random.randint(total_beat // 4, total_beat // 2) # total_beat의 1/4 ~ 1/2 범위에서 랜덤한 정수를 sample 변수에 저장
    sample_beat_times = random.sample(beat_times, sample) # beat_times 리스트에서 sample 개수만큼 랜덤하게 샘플링하여 sample_beat_times 리스트에 저장
    idx = 0 # 인덱스 변수 초기화
    while idx < len(sample_beat_times): # sample_beat_times 리스트의 길이만큼 반복
        lane = random.randint(1, 4) # 1~4 범위에서 랜덤한 정수를 lane 변수에 저장
        if lane == 1: # lane이 1일 경우
            t1.append([0, sample_beat_times[idx]]) # t1 리스트에 [0, sample_beat_times[idx]] 추가
        elif lane == 2: # lane이 2일 경우
            t2.append([0, sample_beat_times[idx]]) # t2 리스트에 [0, sample_beat_times[idx]] 추가
        elif lane == 3: # lane이 3일 경우
            t3.append([0, sample_beat_times[idx]]) # t3 리스트에 [0, sample_beat_times[idx]] 추가
        elif lane == 4: # lane이 4일 경우
            t4.append([0, sample_beat_times[idx]]) # t4 리스트에 [0, sample_beat_times[idx]] 추가
        idx += 1 # 인덱스 변수 1 증가
    t1 = sorted(list(set([tuple(item) for item in t1]))) # t1 리스트에서 중복 제거 후 정렬하여 다시 t1에 저장
    t1 = [list(item) for item in t1] # t1 리스트의 모든 요소를 리스트로 변환하여 다시 t1에 저장
    t2 = sorted(list(set([tuple(item) for item in t2]))) # t2 리스트에서 중복 제거 후 정렬하여 다시 t2에 저장
    t2 = [list(item) for item in t2] # t2 리스트의 모든 요소를 리스트로 변환하여 다시 t2에 저장
    t3 = sorted(list(set([tuple(item) for item in t3]))) # t3 리스트에서 중복 제거 후 정렬하여 다시 t3에 저장
    t3 = [list(item) for item in t3] # t3 리스트의 모든 요소를 리스트로 변환하여 다시 t3에 저장
    t4 = sorted(list(set([tuple(item) for item in t4]))) # t4 리스트에서 중복 제거 후 정렬하여 다시 t4에 저장
    t4 = [list(item) for item in t4] # t4 리스트의 모든 요소를 리스트로 변환하여 다시 t4에 저장
...
def rating(n):
    ...
    if abs(a1*9 - rate_data[n-1]) < 950*speed*(h/900) - 200*speed*(h/900):
        last_combo = combo
        miss_anim = 1
        combo = 0
        combo_effect = 0.2
        ...
        perfect_cnt += 1
        rate = 'PERFECT'
    elif abs(a1*9 - rate_data[n-1]) < 100*speed*(h/900):
        combo += 1
        combo_effect = 0.2
        combo_time = Time + 1
        combo_effect2 = 1.3
